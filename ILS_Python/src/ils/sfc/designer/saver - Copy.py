    ...
    def updateChartSteps(self, stepsToDelete):
        ...
        try:
            ...
            for child in self.children:
                ...
                '''
                if childPath[0] == ".":
                    log.tracef("...converting a relative chart path <%s>...", childPath)
                    parentPath = self.chartPath[:self.chartPath.rfind('/')]
                    childPath = parentPath + childPath[1:]
                    log.tracef("...absolute chart path: <%s>", childPath)
