...
from ils.sfc.gateway.api import getIsolationMode, getProviderName, getDatabaseName
            ...
            if not initialized:
                ...
                if monitorActiveCount > 0 and ((timeLimitMin < 0) or (elapsedMinutes < timeLimitMin) or (persistencePending and elapsedMinutes < extendedDuration)) or watchOnly:
                    ...
                    for configRow in config.rows:
                        ...
                        log.tracef("configRow: %s - %s - %s", dataType, str(numericTargetValue), str(stringTargetValue))
                        ...
                        log.tracef("  (%s) The current PV is: %s-%s (%s)", stepName, str(qv.value), str(qv.quality), tagPath)
