...
Tooltip strategy: We only need to provide tooltips if the block runs for an extended time, i.e., there are timed writes.
So no need to include information about immediate writes because they will be done by the time the user hovers over the block.
(There might be an exception here for an immediate ramp which could start immediately but take several minutes)
'''
                    ...
                    if elapsedMinutes <= 0.0:
                        ...
                        if not(timerWasRunning):
                            ...
                            for row in immediateRows:
                                ...
                                updateTooltip(stepScope, row.key, "Wrote")
...
def setTooltips(stepScope, tooltips):
    header = ["status", "key", "tag", "value", "timing", "outputType", "rampTime" ]
    ds = system.dataset.toDataSet(header, tooltips)
    ds = system.dataset.sort(ds, "timing")
    formatTooltips(stepScope, ds)


def updateTooltip(stepScope, key, status):
    ds = stepScope["tooltipDataset"]
    for row in range(ds.rowCount):
        if ds.getValueAt(row, "key") == key:
            ds = system.dataset.setValue(ds, row, "status", status)
    formatTooltips(stepScope, ds)
    
    
def formatTooltips(stepScope, ds):
    stepScope["tooltipDataset"] = ds
    tooltips = []
    for row in range(ds.rowCount):
        status = ds.getValueAt(row, "status")
        val = ds.getValueAt(row, "value")
        tag = ds.getValueAt(row, "tag")
        timing = ds.getValueAt(row, "timing")
        rampTime = ds.getValueAt(row, "rampTime")
        outputType = ds.getValueAt(row, "outputType")
        
        if string.upper(outputType) in ["OUTPUT RAMP", "SETPOINT RAMP"]:
            tooltips.append("%s %s to %s, a %s for %s at %s min" % (status, str(val), tag, outputType, str(rampTime), str(timing)))
        else:
            tooltips.append("%s %s to %s, a %s at %s min" % (status, str(val), tag, outputType, str(timing)))
