...
def createSession(user, chartName, chartRunId, db):
    ...
    if(numUpdated != 1):
        handleUnexpectedError("insert into session db table failed")
...
def updateSessionStatus(chartProperties, status):
    ...
    numUpdated = system.db.runUpdateQuery(sql, db)
    if(numUpdated != 1):
        handleUnexpectedError(chartProperties, "update of session db table failed")
...
def addControlPanelMessage(message, ackRequired, chartRunId, db):
    ...
    numUpdated = system.db.runUpdateQuery(sql, db)
    if(numUpdated != 1):
        handleUnexpectedError("insert into control panel msg db table failed")
    return msgId
...
def acknowledgeControlPanelMessage(msgId, db):
    ...
    numUpdated = system.db.runUpdateQuery(sql, db)
    if(numUpdated != 1):
        handleUnexpectedError("setting ack time in control panel msg table failed")
...
def timeOutControlPanelMessageAck(msgId, db):
    ...
    numUpdated = system.db.runUpdateQuery(sql, db)
    if(numUpdated != 1):
        handleUnexpectedError("setting ack timed out in control panel msg table failed")
