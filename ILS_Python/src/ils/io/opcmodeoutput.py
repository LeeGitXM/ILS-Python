...
class OPCModeOutput(opcoutput.OPCOutput):
    ...
    def writeDatum(self, val, valueType="", confirmTagPath=""):
        ...
        self.normalValueAsFound = readTag(self.path + '/normalValue')
        ...
        returnToNormal = readTag(self.path + '/returnToNormal').value
        
        if returnToNormal and self.normalValueAsFound.quality.isGood():

            def restore(path=self.path + '/normalValue', val=self.normalValueAsFound.value, OPC_LATENCY_TIME=self.OPC_LATENCY_TIME):
