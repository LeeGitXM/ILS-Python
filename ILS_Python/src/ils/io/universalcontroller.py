...
#TODO What about the permissive
#        success, errorMessage = self.checkConfig(self.path + '/permissive')
        ...
        ''' Check that the clamp values are set such that the value to write will satisfy the clamps. '''
        success, errorMessage = self.checkClamps(tagRoot, val, valueType)
        if not(success):
            writeTag(self.path + "/writeStatus", "Failure")
            writeTag(self.path + "/writeErrorMessage", errorMessage)
            log.infof("Aborting write to %s, checkClamps failed due to: %s", tagRoot, errorMessage)
            return False, errorMessage
...
#        self.restorePermissive()
    ...
    def checkClamps(self, tagRoot, val, valueType):
        ''' Perform a really basic check of the configuration of a tag '''
        log.tracef("In %s.checkClamps, checking %s for a %s value of %s", __name__, self.path, valueType, str(val))
        
        if string.upper(valueType) in ["SP", "SETPOINT", "SETPOINT RAMP"]:
            tagPath = self.path + '/sp'
            targetTag = self.spTag
            valueType = 'sp'
        elif string.upper(valueType) in ["OP", "OUTPUT", "OUTPUT RAMP"]:
            tagRoot = self.path + '/op'
